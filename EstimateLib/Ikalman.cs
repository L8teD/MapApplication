    ...
    public abstract class BaseKalman : IKalman
    {
        ...
        protected Vector X_previous {
            get 
            {
                ...
                {
                    return X_estimate.Dublicate();
                }
            } set { X_previous = value; } }
        Matrix eyeMatrix;
        Matrix F_discrete;
        Matrix G_discrete;
        Matrix Q;
        ...
        private void InitX(InsErrors initErrors, Point point, AbsoluteOmega absOmega, EarthModel earthModel, Velocity velocity, Angles angles, bool Init = false)
        {
            ...
            if (Init)
            {
                ...
                X_error[4] = velocityAccuracy.east;
                X_error[5] = velocityAccuracy.north;
            ...
            else
            {
                ...
                //X_error[1] = X_error[1];// / (earthModel.R2 * Math.Cos(angles.heading));
                //X_error[2] = X_error[2];// / earthModel.R1;
                //X_error[3] = X_error[3];

                //X_error[4] = X_error[4] - (velocity.H / earthModel.R2 + absOmega.E * Math.Tan(point.lat)) * X_error[1] + absOmega.H * X_error[3];
                ////X_error[4] = velocityAccuracy.east + (velocity.H / earthModel.R2 + absOmega.E * Math.Tan(point.lat)) * X_error[1] - absOmega.H * X_error[2] + absOmega.N * X_error[3];
                //X_error[5] = X_error[5] - velocity.H / earthModel.R1 * X_error[1] - absOmega.E * X_error[3];
                //X_error[6] = X_error[6];
            }
            X_error[1] = coordAccuracy.longitude;// / (earthModel.R2 * Math.Cos(angles.heading));
            X_error[2] = coordAccuracy.latitude;// / earthModel.R1;
            X_error[3] = coordAccuracy.altitude;
            ...
            X_error[13] = initErrors.gyroTemperatureKoef.first;
            X_error[14] = initErrors.gyroTemperatureKoef.second;
            X_error[15] = initErrors.gyroTemperatureKoef.third;
            ...
            X_error[19] = initErrors.accTemperatureKoef.first;
            X_error[20] = initErrors.accTemperatureKoef.second;
            X_error[21] = initErrors.accTemperatureKoef.third;
        }
        private void InitF(OmegaGyro omegaGyro, AbsoluteOmega absOmega, EarthModel earthModel, Acceleration acceleration, Matrix C, InputAirData airData)
        {
            ...
            F[4, 17] = C[1, 2];
            F[4, 18] = C[1, 3];
            F[4, 19] = C[1, 1] * acceleration.X * resultTemperature;
            F[4, 20] = C[1, 2] * acceleration.Y * resultTemperature;
            F[4, 21] = C[1, 3] * acceleration.Z * resultTemperature;

            F[5, 1] = -(omegaGyro.Z_dot - absOmega.E * absOmega.H);
            ...
            F[5, 17] = C[2, 2];
            F[5, 18] = C[2, 3];
            F[5, 19] = C[2, 1] * acceleration.X * resultTemperature;
            F[5, 20] = C[2, 2] * acceleration.Y * resultTemperature;
            F[5, 21] = C[2, 3] * acceleration.Z * resultTemperature;
            ...
            F[6, 16] = C[3, 2];
            F[6, 17] = C[3, 3];
            F[6, 18] = C[3, 1] * acceleration.X * resultTemperature;
            F[6, 19] = C[3, 2] * acceleration.Y * resultTemperature;
            F[6, 20] = C[3, 3] * acceleration.Z * resultTemperature;
            ...
            F[7, 11] = C[1, 2];
            F[7, 12] = C[1, 3];
            F[7, 13] = C[1, 1] * omegaGyro.X * resultTemperature;
            F[7, 14] = C[1, 2] * omegaGyro.Y * resultTemperature;
            F[7, 15] = C[1, 3] * omegaGyro.Z * resultTemperature;
            ...
            F[8, 11] = C[2, 2];
            F[8, 12] = C[2, 3];
            F[8, 13] = C[2, 1] * omegaGyro.X * resultTemperature;
            F[8, 14] = C[2, 2] * omegaGyro.Y * resultTemperature;
            F[8, 15] = C[2, 3] * omegaGyro.Z * resultTemperature;
            ...
            F[9, 11] = C[3, 2];
            F[9, 12] = C[3, 3];
            F[9, 13] = C[3, 1] * omegaGyro.X * resultTemperature;
            F[9, 14] = C[3, 2] * omegaGyro.Y * resultTemperature;
            F[9, 15] = C[3, 3] * omegaGyro.Z * resultTemperature;
        }
        ...
        private void InitW(InsErrors insErrors)
        {
            ...
            W_SKO = Vector.Zero(6);
            W_SKO[4] = insErrors.gyroNoiseSKO.first;
            W_SKO[5] = insErrors.gyroNoiseSKO.second;
            W_SKO[6] = insErrors.gyroNoiseSKO.third;
            W_SKO[1] = insErrors.accNoiseSKO.first;
            W_SKO[2] = insErrors.accNoiseSKO.second;
            W_SKO[3] = insErrors.accNoiseSKO.third;
        }
        private void InitH(Point point, EarthModel earth, AbsoluteOmega absOmega, Velocity velocity, Angles angles)
        {
            H = Matrix.Zero(6, 21);

            H[1, 1] = 1.0;// / (earth.R2 * Math.Cos(angles.heading));
            H[2, 2] = 1.0;// / earth.R1;
            H[3, 3] = 1.0;
        ...
        private void InitZ(Point point, Velocity velocity, EarthModel earth, InsErrors insErrors, MeasurementsErrors gnssMeasurements)
        {
            ...
            double[] _snsErrorsSKO = new double[] {
                gnssMeasurements.SKO.lon,
                gnssMeasurements.SKO.lat,
                gnssMeasurements.SKO.alt,
                gnssMeasurements.SKO.E,
                gnssMeasurements.SKO.N,
                gnssMeasurements.SKO.H
            };
            Vector snsErrorsSKO = new Vector(_snsErrorsSKO);
            
            double[] _snsErrors = new double[] {
                gnssMeasurements.SKO.lon * gnssMeasurements.noise.lon,
                gnssMeasurements.SKO.lat * gnssMeasurements.noise.lat,
                gnssMeasurements.SKO.alt * gnssMeasurements.noise.alt,
                gnssMeasurements.SKO.E * gnssMeasurements.noise.E,
                gnssMeasurements.SKO.N * gnssMeasurements.noise.N,
                gnssMeasurements.SKO.H * gnssMeasurements.noise.H
            };
        ...
        private void InitZ(Point point, Velocity velocity, EarthModel earth, InsErrors insErrors, MeasurementsErrors svsMeasurements, int x = 1)
        {
            ...
            double[] _svsErrorsSKO = new double[]
            {
                svsMeasurements.SKO.lon,
                svsMeasurements.SKO.lat,
                svsMeasurements.SKO.alt,
                svsMeasurements.SKO.E,
                svsMeasurements.SKO.N,
                svsMeasurements.SKO.H
            };

            Vector svsErrorsSKO = new Vector(_svsErrorsSKO);
        ...
        public void Model(Input input, Parameters parameters, MeasurementsErrors gnssMeasurments, MeasurementsErrors svsMeasurements)
        {
            ...
            X_dot = F * X + G * W;
            //X_error = X_error + X_dot * input.INS.dt;

            InitH(parameters.point, parameters.earthModel, parameters.absOmega, parameters.velocity, parameters.angles);
