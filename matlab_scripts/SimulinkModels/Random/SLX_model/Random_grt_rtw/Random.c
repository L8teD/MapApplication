...
void Random_step(void)
{
  ...
  /* Matfile logging */
  rt_UpdateTXYLogVars(Random_M->rtwLogInfo, (&Random_M->Timing.taskTime0));
...
void Random_initialize(void)
{
  ...
  /* Matfile logging */
  rt_StartDataLoggingWithStartTime(Random_M->rtwLogInfo, 0.0, rtmGetTFinal
    (Random_M), Random_M->Timing.stepSize0, (&rtmGetErrorStatus(Random_M)));
